...
class MultiScaleFusion_0(nn.Module):
    def __init__(self,v_planes=[256,512,1024],hiden_planes=512,scaled=True):
        super().__init__()
        self.up_modules=nn.ModuleList(
            [nn.Sequential(
    ...
    def forward(self, x):
        ...
        bot_feat=self.bot_proj(l)
        return [bot_feat,mid_feat,top_feat]
class MultiScaleFusion_1(nn.Module):
    def __init__(self,v_planes=[256,512,1024],hiden_planes=512,scaled=True):
        super().__init__()
        self.up_modules=nn.ModuleList(
            [nn.Sequential(
                nn.UpsamplingBilinear2d(scale_factor=2) if scaled else nn.Sequential(),
                darknet_conv_0(v_planes[-2]+hiden_planes//2, hiden_planes//2, ksize=1),
                # darknet_conv(hiden_planes//2, hiden_planes//2, 3),
            ),
            nn.Sequential(
                nn.UpsamplingBilinear2d(scale_factor=2) if scaled else nn.Sequential(),
                darknet_conv_0(v_planes[-1], hiden_planes//2, ksize=1),
                # darknet_conv(hiden_planes//2, hiden_planes//2, 3)
            )]
        )
        ...
        self.down_modules=nn.ModuleList(
            [nn.Sequential(
                nn.AvgPool2d(2, 2) if scaled else nn.Sequential(),
                darknet_conv_0(hiden_planes//2+v_planes[0], hiden_planes // 2, ksize=1),
                # darknet_conv(hiden_planes // 2, hiden_planes//2, 3),
            ),
                nn.Sequential(
                nn.AvgPool2d(2, 2) if scaled else nn.Sequential(),
                darknet_conv_0(hiden_planes+v_planes[1], hiden_planes//2, ksize=1),
                # darknet_conv(hiden_planes//2, hiden_planes//2, 3),
            )]
        )
    ...
    def forward(self, x):
        ...
        bot_feat=self.bot_proj(l)
        return [bot_feat,mid_feat,top_feat]
class MultiScaleFusion_2(nn.Module):
    def __init__(self,v_planes=[256,512,1024],hiden_planes=512,scaled=True):
        super().__init__()
        self.up_modules=nn.ModuleList(
            [nn.Sequential(
        ...
        self.top_proj=darknet_conv_0(v_planes[-1]+hiden_planes//2,hiden_planes,1)
        self.mid_proj=darknet_conv_0(v_planes[1]+hiden_planes,hiden_planes,1)
        self.bot_proj=darknet_conv_0(v_planes[0]+hiden_planes//2,hiden_planes,1)
    ...
    def forward(self, x):
        ...
        bot_feat=self.bot_proj(l)
        return [bot_feat,mid_feat,top_feat]
class MultiScaleFusion_3(nn.Module):
    def __init__(self,v_planes=[256,512,1024],hiden_planes=512,scaled=True):
        super().__init__()
        self.up_modules=nn.ModuleList(
            [nn.Sequential(
