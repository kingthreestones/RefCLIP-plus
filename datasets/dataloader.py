...
from utils_refclip.utils import label2yolobox
...
class RefCOCODataSet(Data.Dataset):
    ...
    def load_refs(self, idx):
        ...
        ref1 = self.proc_ref(ref,self.token_to_ix,self.max_token)
        ref2 = self.proc_ref(ref, self.token_to_ix, self.__C.MAX_TOKEN2)
        return ref1,ref2
    ...
    def __getitem__(self, idx):
        ...
        ref_iter1,ref_iter2 = self.load_refs(idx)
        ...
        return \
            torch.from_numpy(ref_iter1).long(), \
            ...
            mask_id,\
            np.array(info_iter), \
            torch.from_numpy(ref_iter2).long()
