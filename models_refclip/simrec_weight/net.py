...
from models_refclip.simrec_weight.head import REChead
...
from models_refclip.visual_encoder_simrec import visual_encoder
    ...
    def frozen(self,module):
        if getattr(module,'module',False):
            ...
            for param in module.parameters():
                param.requires_grad = False
    def forward(self,x,y, det_label=None,seg_label=None,weight=None):
        # if self.training:
        #     x = cp.checkpoint(self.create_custom_forward(self.visual_encoder), x)
        # else:
        #     x = self.visual_encoder(x)
        # if self.training:
        #     y = cp.checkpoint(self.create_custom_forward(self.lang_encoder), y)
        # else:
        ...
        # if self.training:
        #     x = cp.checkpoint(self.create_custom_forward(self.multi_scale_manner), x)
        # else:
        ...
        bot_feats=x[0]
        if self.training:
            loss,loss_det,loss_seg=self.head(top_feats,bot_feats,det_label,seg_label,weight=weight)
