...
class REChead(nn.Module):
    def __init__(
        ...
        # self.bcewithlog_loss = FocalLoss(bcewithlog_loss)
    ...
    def forward(self, xin, yin,labels=None,y_label=None,mixup_targets=None, imgs=None,weight = None,):
        # print(labels)
        outputs = []
        origin_preds = []
        x_shifts = []
    ...
    def get_losses(
        ...
        loss_weight = weight.view(weight.size(0),1,1).expand_as(obj_preds).contiguous().view(-1, 1)
        loss_obj = (
            nn.BCEWithLogitsLoss(reduction="none",weight=loss_weight)(obj_preds.view(-1, 1), obj_targets.clamp(self.__C.LABEL_SMOOTH,1.-self.__C.LABEL_SMOOTH) if self.__C.LABEL_SMOOTH>0. else obj_targets)
        ).sum() / num_fg
        # loss_cls = (
